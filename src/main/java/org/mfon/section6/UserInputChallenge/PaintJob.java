package org.mfon.section6.UserInputChallenge;
...
public class PaintJob {
    ...
    public static int getBucketCount(double width, double height, double areaPerBucket, int extraBuckets) {
        ...
        int numberOfBucketsToBuy = 0;
        double areaOfWall = 0;
        if (width <= 0 || height <= 0 || areaPerBucket <= 0 || extraBuckets < 0){
            return -1;
        }
        while (width > 0 || height > 0 || areaPerBucket > 0 || extraBuckets >= 0){
            areaOfWall = width * height;
            int numberOfBucketsForAreaOfWall = (int) Math.ceil(areaOfWall / areaPerBucket);
            numberOfBucketsToBuy = numberOfBucketsForAreaOfWall - extraBuckets;
            //System.out.println(numberOfBucketsToBuy);
            break;
        }
    ...
    public static int getBucketCount(double width, double height, double areaPerBucket) {
        ...
        while (width > 0 || height > 0 || areaPerBucket > 0){
            double areaOfWall = width * height;
            numberOfBucketsToBuy = (int) Math.ceil(areaOfWall / areaPerBucket);
            //System.out.println(numberOfBucketsToBuy);
            break;
        }
        return numberOfBucketsToBuy;
